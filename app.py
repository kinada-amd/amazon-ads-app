...
# 比較表のセル内デザイン用関数
def style_diff_cells(val):
    if isinstance(val, str) and '(' in val:
        # 広告費(コスト)の判定用に「¥」や「-」の位置を確認
        is_cost = '¥' in val and '広告費' in st.session_state.get('current_col', '')
        diff_str = val.split('(')[1].split(')')[0].replace('¥', '').replace('%', '').replace(',', '')
        try:
            diff_num = float(diff_str)
            if diff_num > 0:
                # 広告費が増えるのは赤、その他は緑
                color = "#d93025" if "広告費" in st.session_state.get('current_col', '') else "#1e7e34"
                bg = "#fce8e6" if "広告費" in st.session_state.get('current_col', '') else "#e6f4ea"
            elif diff_num < 0:
                color = "#1e7e34" if "広告費" in st.session_state.get('current_col', '') else "#d93025"
                bg = "#e6f4ea" if "広告費" in st.session_state.get('current_col', '') else "#fce8e6"
            else:
                return ""
...
try:
    ...
    if view_mode == "通常モード":
        ...
        # 2. タイプ別実績比較テーブル (st.dataframeに統一)
        ...
        for t in g_a.index:
            ...
            row = {
                'タイプ': t,
                'インプレッション': f"{g_a.loc[t,'インプレッション']:,.0f} ({diff['インプレッション']:+,.0f})",
                'クリック数': f"{g_a.loc[t,'クリック数']:,.0f} ({diff['クリック数']:+,.0f})",
                '広告費': f"¥{g_a.loc[t,'広告費']:,.0f} ({'+' if diff['広告費']>=0 else ''}¥{diff['広告費']:,.0f})",
                '注文': f"{g_a.loc[t,'注文']:,.0f} ({diff['注文']:+,.0f})",
                '広告売上': f"¥{g_a.loc[t,'広告売上']:,.0f} ({'+' if diff['広告売上']>=0 else ''}¥{diff['広告売上']:,.0f})",
                'ROAS': f"{roas_a:.0f}% ({roas_a-roas_b:+.1f}%)"
            }
            compare_data.append(row)
        
        df_compare_display = pd.DataFrame(compare_data)
        
        # 広告費のカラム判定のために一時的にセッションを利用（Styler内での判定用）
        st.session_state['current_col'] = '広告費'
        st.dataframe(
            df_compare_display.style.applymap(style_diff_cells, subset=['インプレッション', 'クリック数', '広告費', '注文', '広告売上', 'ROAS']),
